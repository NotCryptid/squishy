...
class CatchphraseCog(commands.Cog):
    lock = False

    group = app_commands.Group(name="catchphrase", description="Manage lines")
    ...
    async def remove(self, interaction: discord.Interaction, index: str):
        ...
        try:
            ...
            await interaction.followup.send("Invalid catchphrase index.", ephemeral=True)
            self.lock = False
            return
        with open(config_path, "w", encoding="utf-8") as config_file:
            json.dump(config, config_file, indent=4)
        print(f"{interaction.user.name} ({interaction.user.id}) removed line \"{removed_quote}\"")
        await interaction.followup.send(f"Removed line: {removed_quote}", ephemeral=True)
        self.lock = False
    ...
    async def on_message(self, message):
        ...
        if f"<@{self.bot.user.id}>" in message.content:
            quotes = config.get("quotes", ["Put your lines here!\n-# If you're a user, nag a manager to add some."])
